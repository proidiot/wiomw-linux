...
void test_print_iface_data_tracker()
{
	note("running test_print_iface_data_tracker");
	...
	if ((tracker = prepare_data_tracker(iface_data_size, nlh, &get_iface_header_cb, &get_iface_attr_cb)) == NULL) {
		...
		const char* expected = "{\"family\":\"AF_UNSPEC\",\"type\":"
			"\"ARPHRD_ETHER\",\"up\":1,\"running\":1,"
			"\"dynamic\":1,\"volatile\":1,\"mac\":"
			"\"01:23:45:67:89:AB\",\"bcast_mac\":"
			"\"FF:FF:FF:FF:FF:FF\",\"mtu\":1500,\"real_iface\":"
			"\"eth0\",\"qdsp\":\"pfifo_fast\",\"name\":\"eth0\","
			"\"blacklisted\":0,\"rx_packets\":0,\"tx_packets\":0,"
			...
			"\"tx_fifo_errors\":0,\"tx_heartbeat_errors\":0,"
			"\"tx_window_errors\":0,\"rx_compressed\":0,"
			"\"tx_compressed\":0,\"last_changed\":";
		char actual[BUFSIZ];

		print_data_tracker(stream, tracker, &print_iface, &print_iface_diff, NULL);
		...
		if (strncmp(expected, actual, strnlen(expected, BUFSIZ)) != 0) {
			fail("print_iface_data_tracker did not match");
			printf("expected: %s\n", expected);
			printf("actual: %s\n", actual);
		} else {
			pass("print_iface_data_tracker matched");
		}
	}
}
